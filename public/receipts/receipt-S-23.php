<?php
    ...
    $items = array(new item("EBI TEMPURA","1","230.00","230.00"),new item("PEANUTS","1","75.00","75.00"),);
    ...
    $invNo = str_pad("S-23",3," ",STR_PAD_LEFT);
    ...
    $totalAmount = str_pad("305.00",32," ",STR_PAD_LEFT);
    ...
    $printer -> setJustification(Printer::JUSTIFY_LEFT);
    $printer -> setEmphasis(true);
    $totalAmount = str_pad("305.00",29," ",STR_PAD_LEFT);
    ...
    $sigName = str_pad("OS-13, ",3," ",STR_PAD_LEFT);
    ...
    $paymentList = array(new item("1 - CASH"," - 500.00","",""),new item("2 - CARD"," - 22.00","",""),);

    foreach ($paymentList as $pi){
        $printer->text($pi);
    }

    $totalAmount = str_pad("523.00",26," ",STR_PAD_LEFT);
    $printer -> text("TOTAL PAYMENTS".$totalAmount."\n");
    $totalAmount = str_pad("218.00",34," ",STR_PAD_LEFT);
    $printer -> text("CHANGE".$totalAmount."\n");
    $printer -> feed(2);

    $colonVatSales = str_pad(":",11," ",STR_PAD_LEFT);
    $vatSalesAmount = str_pad("272.32",8," ",STR_PAD_LEFT);
    $printer -> text("VAT Sales".$colonVatSales.$vatSalesAmount."\n");

    $colonVatAmount = str_pad(":",6," ",STR_PAD_LEFT);
    $vatAmount = str_pad("32.68",8," ",STR_PAD_LEFT);
