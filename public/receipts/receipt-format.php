<?php
        ...
        $items = array(new item("1 PC CHICKEN WITH EXTRA SAUCE","2","23.50","4600.00"),new item("AROZCALDO NI MIKOY SPECIAL","4","3.56","120.00"),);
        $subtotal = new item("Subtotal", "270.00");
        ...
        $invNo = str_pad("S-123",3," ",STR_PAD_LEFT);
        $printer -> text("Inv. No".$invColon.$invNo."\n");

        $serverColon = str_pad(":",5," ",STR_PAD_LEFT);
        $serverName = str_pad("John Doe",3," ",STR_PAD_LEFT);
        ...
        $totalAmount = str_pad("5300.00",28," ",STR_PAD_LEFT);

       

        $printer -> setJustification(Printer::JUSTIFY_LEFT);
   
        $totalAmount = str_pad("5300.00",32," ",STR_PAD_LEFT);
        ...
        $discount = "Q Citipark Discount 20%";
        $discountCols = 40 - strlen($discount);
        $discountAmount = str_pad("250.00",$discountCols," ",STR_PAD_LEFT);
        $printer->text($discount.$discountAmount."\n");

        $printer -> setJustification(Printer::JUSTIFY_LEFT);
        $printer -> setEmphasis(true);
        $totalAmount = str_pad("5300.00",29," ",STR_PAD_LEFT);
        ...
        $paymentItems = array(new item("1 - Cash","500.00","",""),new item("2 - Card","23941029423059 - 1000.00","",""),);

        foreach ($paymentItems as $pi){
            $printer->text($pi);
        }

        $totalAmount = str_pad("1500.00",26," ",STR_PAD_LEFT);
        $printer -> text("TOTAL PAYMENTS".$totalAmount."\n");
        $totalAmount = str_pad("356.00",34," ",STR_PAD_LEFT);
        $printer -> text("CHANGE".$totalAmount."\n");
        $printer -> feed(2);

        $colonVatSales = str_pad(":",11," ",STR_PAD_LEFT);
        $vatSalesAmount = str_pad("340.00",8," ",STR_PAD_LEFT);
        $printer -> text("VAT Sales".$colonVatSales.$vatSalesAmount."\n");

        $colonVatAmount = str_pad(":",6," ",STR_PAD_LEFT);
        $vatAmount = str_pad("40.82",8," ",STR_PAD_LEFT);
