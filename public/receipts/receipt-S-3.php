<?php
        ...
        $items = array(new item("CLASSIC HAM & CHEESE","1","126.00","126.00"),new item("TUNA SANDWICH","1","126.00","126.00"),);
        ...
        $invNo = str_pad("S-3",3," ",STR_PAD_LEFT);
        ...
        $tableName = str_pad("C-2",3," ",STR_PAD_LEFT);
        ...
        $totalAmount = str_pad("252.00",32," ",STR_PAD_LEFT);
        ...
        $printer -> setJustification(Printer::JUSTIFY_LEFT);
        $printer -> setEmphasis(true);
        $totalAmount = str_pad("252.00",29," ",STR_PAD_LEFT);
        ...
        $paymentList = array(new item("1" - "CASH","" - "500.00","",""),new item("2" - "CASH","" - "500.00","",""),);

        foreach ($paymentItems as $pi){
            $printer->text($pi);
        }

        $totalAmount = str_pad("500.00",26," ",STR_PAD_LEFT);
        $printer -> text("TOTAL PAYMENTS".$totalAmount."\n");
        $totalAmount = str_pad("248.00",34," ",STR_PAD_LEFT);
        $printer -> text("CHANGE".$totalAmount."\n");
        $printer -> feed(2);

        $colonVatSales = str_pad(":",11," ",STR_PAD_LEFT);
        $vatSalesAmount = str_pad("225.00",8," ",STR_PAD_LEFT);
        $printer -> text("VAT Sales".$colonVatSales.$vatSalesAmount."\n");

        $colonVatAmount = str_pad(":",6," ",STR_PAD_LEFT);
        $vatAmount = str_pad("27.00",8," ",STR_PAD_LEFT);
