<?php
    ...
    $items = array(new item("CALAMARI","1","210.00","210.00"),new item("EBI TEMPURA","1","230.00","230.00"),new item("PEANUTS","1","75.00","75.00"),);
    ...
    $invNo = str_pad("S-25",3," ",STR_PAD_LEFT);
    ...
    $totalAmount = str_pad("515.00",32," ",STR_PAD_LEFT);
    ...
    $printer -> setJustification(Printer::JUSTIFY_LEFT);
    $printer -> setEmphasis(true);
    $totalAmount = str_pad("459.82",29," ",STR_PAD_LEFT);
    ...
    $sigName = str_pad("OS-15, 15, OS-16, ",3," ",STR_PAD_LEFT);
    ...
    $paymentList = array(new item("1" - "CASH","" - "600.00","",""),);

    foreach ($paymentItems as $pi){
        $printer->text($pi);
    }

    $totalAmount = str_pad("600.00",26," ",STR_PAD_LEFT);
    $printer -> text("TOTAL PAYMENTS".$totalAmount."\n");
    $totalAmount = str_pad("85.00",34," ",STR_PAD_LEFT);
    ...
    $zeroRatedSalesAmount = str_pad("459.82",8," ",STR_PAD_LEFT);
